...
class _RecommendPageState extends State<RecommendPage>
  ...
  List<Widget> buildList() {
    ...
    list.add(buildGrayLine,);
    return list;
  }

  Container get buildGrayLine {
    return new Container(color: Colors.grey.shade200,
  ...
  Widget buildRecommendItem(Map bean) {
    ...
    list.addAll([
      ...
      new Row(children: <Widget>[
        new Padding(child: new Icon(Icons.subscriptions, size: 18.0,),
          padding: new EdgeInsets.fromLTRB(8.0, 8.0, 8.0, 8.0),),
        new Expanded(child: new Text(bean['title'], style: Theme
            .of(context)
            .textTheme
            .title,)),
      ...
      for (var i = 0; i <= (dataS.length - 1) ~/ 2; i++) {
        list.add(
            new Row(
                mainAxisSize: MainAxisSize.min,
                children: _getItems(
                    dataS.sublist(i * 2, i * 2 + 2), bean['sort'])));
      }
    }
  ...
  _getItems(List<Map> list, int sort) {
    ...
    return list.map((e) {
      ...
      var columnList = [ new ClipRRect(
        borderRadius: const BorderRadius.all(
            const Radius.circular(20.0)),
        child: new Container(width: width,
      ...
      if (list.length != 2)
        columnList.add(
            new Align(alignment: Alignment.centerLeft, child: new Text(
              e.containsKey('authors') ? e['authors'] :
              e['sub_title'], style: Theme
                .of(context)
                .textTheme
                .caption,),));
      ...
      return new GestureDetector(
        child: new Column(children: columnList,), onTap: () {
        if (list.length != 2) {
